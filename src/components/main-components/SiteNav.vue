    ...
    <div class="nav--glow--ani">
      ...
      <div class="nav-container theme-border theme-shadow">
        <div class="auto-margin wrap-nav-btn">
          <button
            id="navBtn"
            class="nav-btn theme-border theme-shadow"
            title="Open / Close Nav"
        ...
        <div id="wrapTitle" class="wrap-title">
          <span class="filter-brightness">
            <img
              class="logo-flip filter-shadow"
              src="@/assets/images/theThinkerIconFlip.png"
              aria-label="[Image of The Thinker Statue]"
            />
          </span>
          <router-link
            class="title-anchor"
          ...
          <span class="filter-brightness">
            <img
              id="logo"
              class="logo"
              src="@/assets/images/theThinkerIcon.png"
              aria-label="[Image of The Thinker Statue]"
            />
          </span>
        </div>
        ...
        <div id="loginLinks" class="wrap-login-links">
          <div
            id="loginBtn"
            class="login-btn"
            title="Open / Close Login and Account Links"
          >
            <a
              role="button"
              class="login-btn_a"
            ...
            <a
              ...
              class="login-btn_a"
              v-else
              aria-label="Login and Account Links"
              >Log In<i class="down-arrow"></i
            ></a>
            <ul
              id="login"
              data-mobile
              class="nav-drop-downs ndd--is--closed theme-border theme-shadow"
              style="display: none"
            >
              <li v-if="isLoggedIn" data-move class="nav-drop-downs_li">
                <a
              ...
              <li v-if="isLoggedIn" data-move class="nav-drop-downs_li">
                <a
                  class="nav-drop-downs_li_a"
                  @click="store.dispatch('logout', null, { root: true })"
                  aria-label="Log Out"
                  ><u>Log Out</u></a
                >
              </li>
      ...
      <div id="navLinksWrap" class="wrap-nav-links">
        <ul
          id="navLinks"
          class="nav-links nav-links--is--closed theme-border theme-shadow"
          title="main navigation links"
        >
          <li class="auto-margin nav-links_li">
            <a role="button" class="nav-links_li_a"
              >Home<i class="down-arrow"></i
            ></a>
            <ul
              class="nav-drop-downs ndd--is--closed theme-border theme-shadow"
              style="display: none"
...
const redirect = () => {
  window.location.assign(
    process.env.VUE_APP_IDMANAGEMENT_BASE_URL + "/Account/Login"
  );
};

const showHideNav = (evt: Event): void => {
  setTimeout(() => {
    if (navLinks.classList.contains("nav-links--is--open")) openCloseNav(evt);
  }, 150);
...
const openCloseNavEventListener = (evt: Event) => {
  if ((evt as AnimationEvent).animationName === "close--nav--links") {
    navLinksWrap.style.display = "none";
    navLinksWrap.removeEventListener(
      "animationend",
      openCloseNavEventListener,
      true
    );
  ...
  document.querySelectorAll(".nav-drop-downs").forEach((element) => {
    if (clickedElement !== null) {
      if (
        ...
        element.classList.contains("ndd--is--open")
      ) {
...
const nddOpenClose = (evt: Event): void => {
  ...
  if ((evt.target as HTMLElement).className === "down-arrow") {
    ...
    (evt.target as HTMLElement).parentElement!.parentElement! instanceof
      HTMLUListElement &&
    (
      evt.target as HTMLElement
    ).parentElement!.parentElement!.classList.contains("nav-drop-downs")
  ) {
    ulDropDown = (evt.target as HTMLElement).parentElement!.parentElement!;

    // event target was login button
  } else if (
    (evt.target as HTMLElement).getAttribute("id") === "loginBtn" ||
    (evt.target as HTMLElement).parentElement!.getAttribute("id") ===
      "loginBtn" ||
    ...
    if (
      navLinks.classList.contains("nav-links--is--open") &&
    ...
    (evt.target as HTMLElement).lastElementChild?.classList.contains(
      "nav-drop-downs"
    )
  ) {
  ...
  if (
    ulDropDown.classList.contains("ndd--is--open") &&
    !ulDropDown.hasAttribute("data-mobile")
  ) {
    ulDropDown.addEventListener(
      "animationend",
      nddOpenCloseEventListener,
      true
    );
  }
...
const nddOpenCloseEventListener = (evt: Event) => {
  if ((evt as AnimationEvent).animationName === "close--ndd") {
    openCloseNav(evt);
    (evt.target! as HTMLElement).removeEventListener(
      "animationend",
      nddOpenCloseEventListener,
      true
    );
